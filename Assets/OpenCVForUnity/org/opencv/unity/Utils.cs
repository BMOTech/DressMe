...
//												}
			...
			colorsHandle.Free ();
				...
				{
						#if (UNITY_ANDROID || UNITY_IPHONE) && !UNITY_EDITOR
						return OpenCVForUnity_GetFilePath (filename);
						#else
						return null;
						#endif
				}
				...
				public static extern string OpenCVForUnity_GetFilePath (string filename);

		[DllImport("opencvforunity", CallingConvention = CallingConvention.Cdecl)]
		public static extern void OpenCVForUnity_MatToTexture (IntPtr mat, IntPtr textureColors);

		[DllImport("opencvforunity", CallingConvention = CallingConvention.Cdecl)]
		public static extern void OpenCVForUnity_TextureToMat (IntPtr textureColors, IntPtr Mat);
